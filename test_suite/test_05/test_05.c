...
uint8_t flags_server[10];
uint8_t flags_client[10];
    ...
    while ((packet = csp_read(conn, 50)) != NULL) {
      ...
      case MY_SERVER_PORT:
        ...
        if (server_received < 10) {
          flags_server[server_received] = csp_conn_flags(conn);
          csp_print("server flags %d\n", csp_conn_flags(conn));
        }
...
void client(void) {
  ...
  while (1) {
    ...
    csp_print("Connection succeded\n");

    if (server_received < 10) {
      flags_client[server_received] = csp_conn_flags(conn);
      csp_print("client flags %d\n", csp_conn_flags(conn));
    }
...
int main(void) {
  ...
  while (1) {
    ...
    for (unsigned i = 0; i < 10; ++i) {
      if (flags_client[i] != flags_server[i]) {
        csp_print("flags do not correspond\n");
        exit(1);
      }
    }

    csp_print("Server received %u packets with the correct flags\n",
              server_received);
    exit(0);
  }
