...
csp_packet_t * csp_read(csp_conn_t * conn, uint32_t timeout) {
	...
	damat_buffer_read[1] = csp_conn_pri_faqas(conn);
	damat_buffer_read[2] = csp_conn_src(conn);    // conn->idin.src;
	damat_buffer_read[3] = csp_conn_dst(conn);    // conn->idin.dst;
	damat_buffer_read[4] = csp_conn_dport(conn);  // conn->idin.dport;
	damat_buffer_read[5] = csp_conn_sport(conn);  // conn->idin.sport;
	damat_buffer_read[6] = csp_conn_flags(conn);  // conn->idin.flags;

	mutate_FM_Read(damat_buffer_read);

	set_conn_idin_pri(conn, damat_buffer_read[1]);
	set_conn_idin_src(conn, damat_buffer_read[2]);
	set_conn_idin_dst(conn, damat_buffer_read[3]);
	set_conn_idin_dport(conn, damat_buffer_read[4]);
	set_conn_idin_sport(conn, damat_buffer_read[5]);
	set_conn_idin_flags(conn, damat_buffer_read[6]);
...
void csp_send(csp_conn_t * conn, csp_packet_t * packet) {
	...
	// printf("probe csp_send\n");

	damat_buffer_send[0] = 0;
	damat_buffer_send[1] = csp_conn_pri_faqas(conn);
	damat_buffer_send[2] = csp_conn_src(conn);    // conn->idin.src;
	damat_buffer_send[3] = csp_conn_dst(conn);    // conn->idin.dst;
	damat_buffer_send[4] = csp_conn_dport(conn);  // conn->idin.dport;
	damat_buffer_send[5] = csp_conn_sport(conn);  // conn->idin.sport;
	damat_buffer_send[6] = csp_conn_flags(conn);  // conn->idin.flags;

	mutate_FM_Send(damat_buffer_send);

	set_conn_idin_pri(conn, damat_buffer_send[1]);
	set_conn_idin_src(conn, damat_buffer_send[2]);
	set_conn_idin_dst(conn, damat_buffer_send[3]);
	set_conn_idin_dport(conn, damat_buffer_send[4]);
	set_conn_idin_sport(conn, damat_buffer_send[5]);
